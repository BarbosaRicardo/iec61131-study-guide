...
import { Menu, X, Zap, BookOpen, BarChart2, Home, Binary, Code, GitBranch, Share2, GitGraph, Package, Wrench, FlaskConical, CreditCard, LayoutGrid, LogIn, LogOut , FileText, ChevronDown, Network, Globe, Code2, Sliders, Server, LayoutDashboard, ScanSearch, GraduationCap, CheckCircle2 } from 'lucide-react'
...
const ICON_MAP = {
  BookOpen, Binary, Code, GitBranch, Share2, GitGraph, Package, Zap, Wrench, FlaskConical, CreditCard,
}
...
export default function Sidebar() {
  ...
  const [streak] = useState(() => {
    try {
      ...
      const lastDate = localStorage.getItem('iec61131_streak_date') || ''
      const cur = parseInt(localStorage.getItem('iec61131_streak') || '0', 10)
      if (lastDate === today) return cur
      const next = lastDate === yesterday ? cur + 1 : 1
      localStorage.setItem('iec61131_streak', String(next))
      localStorage.setItem('iec61131_streak_date', today)
      return next
    } catch { return 1 }
  })
      ...
      <div className="p-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        <div className="flex items-center gap-3">
          <button
            ...
            style={{ background: 'linear-gradient(135deg, #1d4ed8, #3b82f6)', boxShadow: '0 0 20px rgba(59,130,246,0.45)' }}>
            <Zap size={18} className="text-white" />
          </button>
          <div>
            <div className="font-black text-white text-sm tracking-wide leading-tight">IEC 61131-3</div>
            <div className="text-xs font-medium" style={{ color: '#3b82f6' }}>Study Guide</div>
          </div>
        </div>
        ...
        <div className="mt-4">
          <div className="flex justify-between text-xs mb-1.5">
            ...
            <span className="font-bold" style={{ color: '#3b82f6' }}>{prog.pct}%</span>
          </div>
      ...
      <nav className="flex-1 overflow-y-auto p-3 space-y-0.5">
        ...
        <div>
          ...
          {showChapters && (
            ...
            {/* Footer */}
      <div className="p-4 space-y-3" style={{ borderTop: '1px solid rgba(59,130,246,0.12)' }}>
        <a
          ...
          style={{ background: 'linear-gradient(135deg,#1d4ed8,#3b82f6)', boxShadow: '0 0 14px rgba(59,130,246,0.35)' }}
        >
        ...
        <a
          ...
          style={{ background: 'rgba(59,130,246,0.07)', border: '1px solid rgba(59,130,246,0.18)', color: 'rgba(59,130,246,0.7)' }}
        >
          <FileText size={12} />
          Course Syllabus
        </a>
        ...
        <p className="text-center text-xs" style={{ color: 'rgba(59,130,246,0.35)' }}>
          IEC 61131-3 · PLCopen · May 2026
        </p>
      </div>
    </div>
  )
  ...
  return (
    <>
      ...
      {showTraining && <TrainingModal course="iec61131" onClose={() => setShowTraining(false)} />}
    </>
  )
}
